...
if (isset($_GET['id'])) {
  ...
  $sql = "SELECT * FROM bike WHERE bike_id = $product_id";
  $bike_detail = $conn->query($sql);

  if ($bike_detail->num_rows > 0) {
    $product = $bike_detail->fetch_assoc();
  }
}
?>
...
<head>
  ...
  <title><?php echo isset($product['bike_name']) ? $product['bike_name'] : 'Product Detail'; ?></title>
...
<body>
  ...
  <div class="container">
    <div class="row row-st product-container">
      <div class="col-12 col-md mt-4">
        <img src="<?php echo $product['bike_img']; ?>" class="img-fluid product-img" width="500rem" />
      </div>
      <div class="product-detail col-12 col-md d-flex flex-column justify-content-center">
        <h2 class="product-title"><?php echo $product['bike_name']; ?></h2>
        <p class="product-text">&#8377;<?php echo $product['bike_price']; ?> <span>Onward</span></p>
      </div>
      <?php if (isset($_SESSION['email'])) { ?>
        <a href="#" class="btn btn-dark buy-btn" data-bs-toggle="modal" data-bs-target="#bike-detail-modal">Buy Now</a>
      <?php } else { ?>
    ...
    <div class="row row-st feature-box">
      ...
      <div class="col feature-item text-center">
        ...
        <p class="spec-detail"><?php echo $product['bike_engine']; ?></p>
      </div>
      <div class="col feature-item text-center">
  ...
  <div class="modal fade" id="bike-detail-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered justify-content-center">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <h3 class="text-center modal-buy-subheading">Bike Details</h3>
          <p class="modal-buy-detail">Bike Name: <?php echo $product['bike_name']; ?></p>
          <p class="modal-buy-detail">Bike Price: <?php echo $product['bike_price']; ?></p>
          <p class="modal-buy-detail">Bike Engine: <?php echo $product['bike_engine']; ?></p>
          <p class="modal-buy-detail">Bike Fuel Type: <?php echo $product['fuel_type']; ?></p>
          <p class="modal-buy-detail">Bike Kerb Weight: <?php echo $product['kerb_type']; ?></p>
          <p class="modal-buy-detail">Bike Mileage: <?php echo $product['mileage']; ?></p>
          ...
          <a href="#" class="confirm-buy-link bike-buy-link text-center">Confirm Buy</a>
  ...
  <script>
    $(".bike-buy-link").click(function(e) {
      e.preventDefault();
      $.ajax({
        type: "GET",
        url: "insert_bike_sale.php",
        data: {
          bike_id: <?php echo $_GET['id'] ?>
        },
        dataType: "JSON",
